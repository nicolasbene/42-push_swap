...
# sert a trouver toutes les permutations d\'une chaine de caractere
perm() {
	items="$1"
	out="$2"
...
# clacul la factorielle de l argument donne 
fact() {
...
checkallperms() {
	s=0
	for n in $(perm $1) # pour chaque permutation de la list perm $1
	do
		a=$(echo $n | sed 's/./& /g')
		b=$(echo ./push_swap $a | tr '\0' ' ') #pour enlever le \0 de bash
		c=$(./push_swap $a | ./checker_linux $a)
		o=$($b |wc -l) #pour compter le nombre de ligne donc le nmbre dope
		if [ $o -gt $2 ] || [ "$c" != "OK" ]; then
